...
/// @param _command_list
function _tby_branch_nested_array_add(argument0, argument1) {
	...
	for (var i = 0; i < tby_arrlen(_command_list); i++) {
	    ...
	    if (is_array(_array_entry) == false) {
			...
			if (_tby_array_is_shallow(_array_entry)) {
	    		...
	    		var _flattened_entry = _tby_array_flatten_shallow(_array_entry);
	            ...
	            for (var j = 0; j < tby_arrlen(_flattened_entry); j++) {
	                ...
	                _tby_list_add(_list, _flat_entry);
	            }
			}
	    }
	}
